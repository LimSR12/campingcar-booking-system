...
public class RentalDelete extends JDialog {
    private final RentalDao dao;
    private final Runnable  onRefresh;
    private final JTextField tfCond = new JTextField(30);

    public RentalDelete(Frame p, RentalDao dao, Runnable ref) {
        super(p, "조건식에 해당하는 rental 삭제", true);
    ...
    private void init() {
        ...
        JButton del=new JButton("삭제"), canc=new JButton("취소");
        btn.add(del); btn.add(canc); root.add(btn); getRootPane().setDefaultButton(del);

        del.addActionListener(e->execute());
